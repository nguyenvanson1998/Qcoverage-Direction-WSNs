...
class Individual:
    def __init__(self, dim: int, max_active):
        self.dim = dim
    ...
    def copy(self):
        p = Individual(self.dim,self.max_active)
        p.dim = self.dim
        p.active = np.copy(self.active)
        p.alpha = np.copy(self.alpha)
        return p
    def fix_gen(self):
        ind_actived = np.where(self.active == True)[0]
        #print('hihi', ind_actived)
        if len(ind_actived) > self.max_active:
            numfalse = len(ind_actived) - self.max_active
            choosed = np.random.choice(ind_actived, numfalse, replace=False)
            #print('len choosed', len(choosed))
            self.active[choosed] = False
            #print('available active', len(np.where(self.active == True)[0]))
